import pygame
from trainedq import get_q
#from main_ai import
Q_walk, Q_jump, Q_attack = get_q("0.1_0.8")
...
class FighterAI(Fighter):
    ...
    def move(self, screen_width, screen_height, surface, target, state):
        ...
        # jump
        if False: #(state["being_attacked"] == True) and (random.randint(0,100) == 0):
            n_jump = 1
        else:
            n_jump = 0
        ...
        try:
            return {"walk": walk, "jump": jump, "attack": attack}
        except UnboundLocalError:
            return {"walk": 0, "jump": 0, "attack": 0}
